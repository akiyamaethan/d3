...
type CellMemento = { tokenValue: number | null };
...
function setCellTokenValue(i: number, j: number, tokenValue: number | null) {
  modifiedCells.set(cellKey(i, j), { tokenValue });
}
...
function startNewGame() {
  ...
  // clear visible cells
  for (const [, rect] of cellLayers) map.removeLayer(rect);
  for (const [, marker] of valueMarkers) map.removeLayer(marker);
  cellLayers.clear();
  valueMarkers.clear();
